# session-bridge-widget.js / portal-url.js

Docs for `session-bridge-widget.js` and `portal-url.js` — see the repo's
[README.md](README.md) for how this fits alongside the other scripts here.

Source of truth for the session bridge / login modal widget embedded on the
TreeSpoke storefront (Webflow), which is also the single source of portal
auth state for the whole storefront (login UI, sign-out, chat toasts, and
any deep-link button).
